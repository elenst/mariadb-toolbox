...
################################################################################
query:
	{ @nonaggregates = () ; $tables = 0 ; $fields = 0 ; $subquery_idx=0 ; $child_subquery_idx=0 ; "" } FLUSH STATUS; query2 ;

query2:

	main_select | main_select | main_select | main_select | create_drop ;

create_drop:

	CREATE temporary TABLE IF NOT EXISTS _tmptable[invariant] AS main_select[invariant] ; INSERT INTO _tmptable[invariant] main_select[invariant] ; DROP TABLE IF EXISTS _tmptable[invariant] ; 	
...
int_double_member_subquery:
    ...
    ( SELECT distinct select_option subquery_table_one_two . int_field_name AS { "SQ".$subquery_idx."_field1" } , 
      ...
      subquery_having 
      limit_rows_examined_clause ) |
    (  SELECT _digit , _digit limit_rows_examined_clause UNION all_distinct  SELECT _digit, _digit limit_rows_examined_clause ) ;
...
char_single_union_subquery:
    (  SELECT _char limit_rows_examined_clause UNION all_distinct  SELECT _char limit_rows_examined_clause )  ;

char_single_union_subquery_disabled:
    char_single_member_subquery limit_rows_examined_clause UNION all_distinct char_single_member_subquery limit_rows_examined_clause ;
...
char_double_member_subquery:
   ...
   ( SELECT distinct select_option subquery_table_one_two . char_field_name AS { "SQ".$subquery_idx."_field1" } ,
     ...
     subquery_having 
     limit_rows_examined_clause ) |
   (  SELECT _char , _char limit_rows_examined_clause UNION all_distinct  SELECT _char , _char limit_rows_examined_clause ) ;
...
int_single_union_child_subquery:
    (  SELECT _digit  UNION all_distinct  SELECT _digit limit_rows_examined_clause  )  ;
...
order_by_clause:
	| | |
        ORDER BY alias1 . _field_indexed desc , total_order_by  limit2 |
	ORDER BY order_by_list |
	ORDER BY  order_by_list, total_order_by limit2 ;
...
limit_rows_examined_clause:
	| LIMIT ROWS EXAMINED _smallint_unsigned;
...
limit2:
      ...
      limit_rows_examined_clause | 
      limit_rows_examined_clause | 
      limit limit_rows_examined_clause | 
      limit_rows_examined_clause limit ;
