...
query_init:
	...
	CREATE VIEW v_customer AS SELECT * FROM customer ; CREATE VIEW v_lineitem AS SELECT * FROM lineitem ; CREATE VIEW v_nation AS SELECT * FROM nation ; CREATE VIEW v_orders AS SELECT * FROM orders ; CREATE VIEW v_part AS SELECT * FROM part ; CREATE VIEW v_partsupp AS SELECT * FROM partsupp ; CREATE VIEW v_region AS SELECT * FROM region ; CREATE VIEW v_supplier AS SELECT * FROM v_supplier ;
...
join_condition_item:
    ...
    current_table_item . `col_char1` = previous_table_item . char_field_name on_subquery |
    current_table_item . char_field_name = previous_table_item . `col_char1` on_subquery ;
...
subquery_join_condition_item:
    ...
    subquery_current_table_item . `col_char1` = subquery_previous_table_item . char_field_name subquery_on_subquery |
    subquery_current_table_item . char_field_name = subquery_previous_table_item . `col_char1` subquery_on_subquery ;
...
child_subquery_join_condition_item:
    ...
    child_subquery_current_table_item . `col_char1` = child_subquery_previous_table_item . char_field_name |
    child_subquery_current_table_item . char_field_name = child_subquery_previous_table_item . `col_char1` ;
...
range_predicate1_item:
         ...
         alias1 . `col_char1` arithmetic_operator _char[invariant]  |
         alias1 . int_indexed not IN (number_list) |
         alias1 . `col_char1` not IN (char_list) |
         alias1 . `pk1` > _tinyint_unsigned[invariant] AND alias1 . `pk1` < ( _tinyint_unsigned[invariant] + _tinyint_unsigned ) |
...
range_predicate2_item:
        alias1 . `pk1` = _tinyint_unsigned |
        alias1 . `col_int_key` = _tinyint_unsigned |
        alias1 . `col_char1` = _char |
        alias1 . int_indexed = _tinyint_unsigned |
        alias1 . `col_char1` LIKE CONCAT( _char , '%') |
        alias1 . int_indexed = existing_table_item . int_indexed |
        alias1 . `col_char1` = existing_table_item . `col_char1` ;
...
order_by_item:
        alias1 . _field_indexed , existing_table_item .`pk1` desc  |
...
view:
    v_customer | v_lineitem | v_nation | v_orders | v_part | v_partsupp | v_region | v_supplier ;
