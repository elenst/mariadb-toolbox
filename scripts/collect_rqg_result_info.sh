...
for t in $test_log ; do
    ...
    echo ""
    grep -aE '^#.*\[ERROR\]|^#.*\[FATAL ERROR\]|DATABASE_CORRUPTION|ALARM|ENVIRONMENT|CRITICAL|REPLICATION_FAILURE|runall.*exited with exit status|runall.*will exit with exit status|Test run ends with exit status|MemoryUsage monitor|FeatureUsage detected|^mysqldump|^mariadb-dump' $t | grep -vE "Executor::MariaDB::execute: Failed to reconnect after getting|Discarding query|undefined value as an ARRAY reference"
    echo ""
    echo ""
