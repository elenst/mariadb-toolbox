...
for v in $vardir ; do
    ...
    for l in $error_logs ; do
        ...
        echo ""
        grep -Ei '^Version: |assertion|signal |\[FATAL\]|ERROR|pure virtual method|safe_mutex:|overflow|overrun|0x|^Status: |Forcing close of thread|Warning.*Thread.*did not exit|InnoDB: A long semaphore wait|WSREP: Server paused at|free\(\): invalid size' $l | grep -v "Invalid roles_mapping table entry user" | grep -v "Can't open and lock privilege tables"
        echo "---------------------------------------------------------"
        echo ""
    done
