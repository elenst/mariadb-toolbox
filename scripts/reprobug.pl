...
if ($rqg_result != 0) {
    ...
    register_repro_stage("FAILED (on RQG simplification)");
    exit 1;
}
...
if ($result == 0) {
    ...
    register_repro_stage("SUCCEEDED (on RQG simplification)");
}
exit 0;
...
sub mtr_simplification {
    ...
    my $cmd1= "cd $basedir/mysql-test; perl $scriptdir/simplify-mtr-test.pl --initial-trials=1 --suitedir=$suitedir --testcase=$testname --options=\"$cnf_options @mtr_options @mtr_timeouts\" --output=\"$output\"";
